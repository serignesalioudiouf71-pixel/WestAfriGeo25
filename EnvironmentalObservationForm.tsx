...
const initialFormData = {
    ...
    observer: '',
    location: '',
    observationType: 'waste_management',
    description: '',
    correctiveActions: '',
};

const EnvironmentalObservationForm: React.FC = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        console.log("Environmental Observation Submitted:", formData);
        alert("Observation environnementale enregistrée ! (Voir console)");
        setFormData(initialFormData);
    };

     const handleDownload = () => {
        downloadData([formData], 'environmental_observation.csv');
    };

    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <FormSection title="Fiche d'Observation Environnementale">
                <FormField label="Date" id="date" name="date" type="date" value={formData.date} onChange={handleChange} />
                <FormField label="Observateur" id="observer" name="observer" value={formData.observer} onChange={handleChange} />
                <FormField label="Lieu / Zone" id="location" name="location" value={formData.location} onChange={handleChange} className="md:col-span-2" placeholder="ex: Zone de stockage carburant, rig site, etc." />
                <FormField
                    ...
                    options={[
                        ...
                        { value: 'spill', label: 'Déversement (hydrocarbures, produits chimiques)' },
                        { value: 'erosion', label: 'Érosion / Sédimentation' },
                        { value: 'fauna_flora', label: 'Faune / Flore' },
            ...
            <FormSection title="Description et Actions">
                <FormField label="Description détaillée de l'observation" id="description" name="description" as="textarea" value={formData.description} onChange={handleChange} className="md:col-span-2" />
                <FormField label="Actions correctives (prises ou suggérées)" id="correctiveActions" name="correctiveActions" as="textarea" value={formData.correctiveActions} onChange={handleChange} className="md:col-span-2" />
            </FormSection>
...
export default EnvironmentalObservationForm;